...
if ($_POST['funcion'] == 'generar_examen') {
   ...
   foreach ($examen->objetos as $objeto) {
       ...
       $pdf->SetFont('Arial', '', 12, '', true); // Add the fifth parameter as true to enable UTF-8
       $pdf->Cell(45, 10, $option1.'.- '.$objeto->respuesta_f, 0, 0, 'L'); // Imprime la primera respuesta
       $pdf->Cell(45, 10, $option2.'.- '.$objeto->respuesta_f2, 0, 0, 'L'); // Imprime la segunda respuesta
       $pdf->Cell(45, 10, $option3.'.- '.$objeto->respuesta_c, 0, 1, 'L'); // Imprime la tercera respuesta y avanza a la siguiente línea
   }
