...
include_once 'conexion.php';
    class Examen {
        ...
        WHERE temas.asignatura LIKE :consulta AND temas.parcial LIKE :parcial AND temas.id IN (";

$sql .= implode(', ', $checkedDataIds);

$sql .= ") ORDER BY RAND() LIMIT 25"; // adjust the limit to the desired number of random preguntas
                $query = $this->acceso->prepare($sql);
                $query->execute(array(':consulta' => "%$consulta%", ':parcial' => "%$parcial%"));
                $this->objetos=$query->fetchall();
                return $this->objetos;
